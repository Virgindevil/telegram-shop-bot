...
async def update_stock_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if str(user_id) != ADMIN_ID:  # Замените на ID администратора
...
async def view_all_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
     # Проверяем, откуда пришёл запрос — из message или callback_query
    if update.message:
        user_id = update.message.from_user.id
        chat = update.message
        ...
        return  # Неизвестный тип входящего сообщения

    # Проверяем, является ли пользователь администратором
    if str(user_id) != ADMIN_ID:  # замените на ваш ID администратора
        await chat.reply_text("У вас нет прав для выполнения этой команды.")
        return

    # Получаем номер страницы из контекста или устанавливаем первую
    ...
    # Разбиваем заказы по страницам (по 5 заказов на странице)
    PAGE_SIZE = 5
    ...
    # Формируем текст с информацией о заказах на текущей странице
    orders_text = ""
    ...
    # Удаление старых сообщений и файлов
    ...
    if "sent_files" in context.user_data:
        for file_type, file_id in context.user_data["sent_files"]:
            try:
                if file_type == "photo":
                    await context.bot.delete_message(chat_id=chat.chat_id, message_id=file_id)
                elif file_type == "document":
                    await context.bot.delete_message(chat_id=chat.chat_id, message_id=file_id)
            except Exception as e:
                print(f"Ошибка при удалении файла: {e}")
        context.user_data["sent_files"] = []

    # Отправка файлов из текущей страницы
    sent_files = []

    for order in current_page_orders:
        _, _, _, _, _, _, _, payment_proof, payment_proof_type = order
        if payment_proof:
            try:
                if payment_proof_type == 'photo':
                    msg = await context.bot.send_photo(chat_id=chat.chat_id, photo=payment_proof)
                    sent_files.append(("photo", msg.message_id))
                elif payment_proof_type == 'document':
                    msg = await context.bot.send_document(chat_id=chat.chat_id, document=payment_proof)
                    sent_files.append(("document", msg.message_id))
            except Exception as e:
                print(f"Ошибка при отправке файла: {e}")

    context.user_data["sent_files"] = sent_files
    ...
    if len(orders) > PAGE_SIZE:
        buttons_row = []
        if page > 1:
            buttons_row.append(InlineKeyboardButton("⬅️ Пред. страница", callback_data="prev_orders_page"))
        if end_index < len(orders):
            buttons_row.append(InlineKeyboardButton("След. страница ➡️", callback_data="next_orders_page"))
        if buttons_row:
            keyboard.append(buttons_row)

    # Добавляем кнопку "Закрыть"
    keyboard.append([InlineKeyboardButton("❌ Закрыть", callback_data="close_orders")])
    
    reply_markup = InlineKeyboardMarkup(keyboard)

    # Отправляем новое сообщение
    try:
        message = await chat.reply_text(
            text=f"🛒 Список всех заказов (Страница {page} из {total_pages}):\n\n{orders_text}",
    ...
    # Сохраняем message_id и номер страницы
    context.user_data["last_message_id"] = message.message_id
    context.user_data["view_all_orders_page"] = page
...
async def navigate_orders_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "next_orders_page":
        context.user_data["view_all_orders_page"] = page + 1
    elif action == "prev_orders_page":
        context.user_data["view_all_orders_page"] = page - 1

    await view_all_orders(update, context)

# Кнопка закрыть список заказов (админ)
async def close_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Удаляем последнее сообщение с заказами
    if "last_message_id" in context.user_data:
        try:
    ...
    # Удаляем все файлы (фото/документы)
    if "sent_files" in context.user_data:
        for file_type, file_id in context.user_data["sent_files"]:
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=file_id)
            except Exception as e:
                print(f"Ошибка при удалении файла: {e}")
        del context.user_data["sent_files"]

    # Не переходим никуда — просто завершаем действие
    try:
...
def add_test_products():
    ...
    if product_count == 0:
        ...
        ("Осеннее настроение (Сахар)", 350, 10, "1.jpg", 1),
...
async def show_products_by_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    page = context.user_data.get("category_page", {}).get(f"category_{category_id}", 1)
    PAGE_SIZE = 3
    ...
    if not products:
        message = await query.message.reply_text("В этой категории товаров нет.")
        context.user_data[user_id]["last_message_id"] = message.message_id
        return

    product_list = list(products.items())
    total_pages = (len(product_list) + PAGE_SIZE - 1) // PAGE_SIZE
    start_index = (page - 1) * PAGE_SIZE
    current_page_products = product_list[start_index:start_index + PAGE_SIZE]
    ...
    # Удаляем текстовое сообщение ("Выберите товар:")
    if "last_message_id" in context.user_data.get(user_id, {}):
        try:
            await context.bot.delete_message(
                chat_id=query.message.chat_id,
    ...
    # Отправляем товары
    product_messages = []
    for product_id, product in current_page_products:
    ...
    if page > 1:
        nav_buttons.append(InlineKeyboardButton("⬅️ Пред. страница", callback_data=f"prev_category_page_{category_id}"))
    if start_index + PAGE_SIZE < len(products):
        nav_buttons.append(InlineKeyboardButton("След. страница ➡️", callback_data=f"next_category_page_{category_id}"))
    if nav_buttons:
        keyboard.append(nav_buttons)

    keyboard.append([InlineKeyboardButton("Назад к категориям", callback_data="view_products")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    message = await query.message.reply_text("Выберите товар:", reply_markup=reply_markup)
    
    # Сохраняем message_id нового текстового сообщения
    context.user_data[user_id]["last_message_id"] = message.message_id
    context.user_data[user_id]["product_messages"] = product_messages
    context.user_data.setdefault("category_page", {})[f"category_{category_id}"] = page
...
# Переклоючение страниц с товарами
async def navigate_category_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    action = data_parts[0]  # 'prev' или 'next'
    category_id = data_parts[-1]

    current_page = context.user_data.get("category_page", {}).get(f"category_{category_id}", 1)
    products = get_products_by_category(category_id)
    PAGE_SIZE = 3
    ...
    context.user_data["category_page"][f"category_{category_id}"] = new_page
...
async def back_to_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in context.user_data:
        context.user_data[user_id] = {"cart": {}, "product_messages": [], "reserved_stock": {}}
...
async def add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):   
    ...
    if user_id not in context.user_data:
        ...
        # Проверяем и инициализируем при необходимости
        if "cart" not in context.user_data[user_id]:
            context.user_data[user_id]["cart"] = {}
        if "reserved_stock" not in context.user_data[user_id]:
            context.user_data[user_id]["reserved_stock"] = {}
    ...
    reserved_stock = context.user_data[user_id]["reserved_stock"]
...
def main():
    ...
    ADMIN_CHAT_ID = ADMIN_ID  # Замените на ID администратора
    ...
    application.add_handler(CallbackQueryHandler(navigate_orders_page, pattern="^prev_orders_page$|^next_orders_page$")) # Обработчик заказов по страницам 
    ...
    application.add_handler(CallbackQueryHandler(close_orders, pattern="^close_orders$")) # Обработчик кнопки "Закрыть" (товары Админ)
    application.add_handler(CallbackQueryHandler(view_products, pattern="^view_products$")) # Показать категории товаров
    application.add_handler(CallbackQueryHandler(navigate_category_page, pattern=r"^(prev|next)_category_page_\d+$")) # Обработчик товаров по страницам 
